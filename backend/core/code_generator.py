...
def execute_commands_with_tello(commands: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(battery, (int, float)) or battery < 20:
        ...
        # Atterrissage de securite : si l'exception est survenue alors que le
        # drone etait en vol (par exemple entre un takeoff et un land), on tente
        # de le poser plutot que de le laisser en stationnaire.
        if airborne:
            logs.append("safety land: attempting emergency landing")

            try:
                tello.land()
                airborne = False
                logs.append("safety land: ok")

            except Exception as land_error:
                logs.append(f"safety land: FAILED ({land_error})")
