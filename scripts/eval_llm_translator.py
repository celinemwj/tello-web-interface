"""
Evaluation du traducteur LLM (scenarios reels contre l'API Gemini).

ATTENTION : ce script n'est PAS un test unitaire.
Il appelle reellement l'API Gemini, necessite donc une cle GEMINI_API_KEY
valide, consomme des tokens, et son resultat peut varier d'une execution a
l'autre puisque la sortie du modele n'est pas deterministe.

Les tests unitaires reproductibles (sans cle ni drone) sont dans tests/.
